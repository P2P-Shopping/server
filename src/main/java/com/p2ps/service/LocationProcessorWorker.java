...
public class LocationProcessorWorker {
    ...
    @Transactional // Foarte important pentru a lega operațiile într-o singură tranzacție
    public void processAndCalculateCenters() {
        System.out.println("⏳ [Worker] Începem recalcularea centrelor absolute...");

        try {
            // 1. Creăm/golim tabelul staging
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS store_inventory_map_staging (LIKE store_inventory_map INCLUDING ALL)");
            jdbcTemplate.execute("TRUNCATE store_inventory_map_staging");

            // 2. Rulăm algoritmul DBSCAN + Geometric Median în tabelul staging
            String sql = """
                INSERT INTO store_inventory_map_staging (store_id, item_id, estimated_loc_point, confidence_score, ping_count)
            ...
            // Executăm inserția în staging
            int rowsAffected = jdbcTemplate.update(sql);

            // 3. Swap atomic: ștergem datele vechi și inserăm cele noi într-o singură tranzacție
            jdbcTemplate.execute("DELETE FROM store_inventory_map");
            jdbcTemplate.execute("INSERT INTO store_inventory_map SELECT * FROM store_inventory_map_staging");
